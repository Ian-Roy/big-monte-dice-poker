  ...
  <div class="title-page" aria-label="Monte's Delux Dice Poker title screen">
    <div class="title-card">
      ...
      <div class="title-actions">
        ...
        <button
          type="button"
          class="title-btn tertiary"
          :disabled="!hasSavedGames"
          @click="$emit('load-games')"
        >
          Load game
        </button>
      </div>
      ...
      <div class="title-utilities">
        ...
        <button
          ...
          @click="handleInstall"
        >
          {{ pwaBusy ? 'Opening…' : installButtonLabel }}
        </button>
        <button
          ...
          @click="handleRefresh"
        >
          {{ pwaBusy ? 'Refreshing…' : 'Refresh PWA (update ready)' }}
        </button>
      </div>
      <p v-if="pwaMessage" class="title-status">{{ pwaMessage }}</p>
      <p v-else-if="installHint" class="title-status">{{ installHint }}</p>
...
const { canInstallPwa, installMode, updateAvailable } = usePwaState();
const pwaBusy = ref(false);
const pwaMessage = ref('');

const installButtonLabel = computed(() => {
  if (installMode.value === 'ios-manual') return 'Install on iPhone/iPad';
  if (installMode.value === 'ios-open-safari') return 'Install in Safari';
  return 'Install PWA';
});

const installHint = computed(() => {
  if (installMode.value === 'ios-manual') {
    return 'On iPhone/iPad Safari: tap Share, then Add to Home Screen.';
  }
  if (installMode.value === 'ios-open-safari') {
    return 'Open this site in Safari, then use Share → Add to Home Screen.';
  }
  return '';
});
...
async function handleInstall() {
  ...
  try {
    ...
    else if (result === 'ios-manual') pwaMessage.value = 'Tap Share, then Add to Home Screen.';
    else if (result === 'ios-open-safari') pwaMessage.value = 'Install works in Safari on iOS. Open there first.';
