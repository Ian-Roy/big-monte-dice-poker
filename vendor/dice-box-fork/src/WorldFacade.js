...
const defaultOptions = {
  ...
  delay: 10, // delay between dice being generated - 0 causes stuttering and physics popping
	...
	offscreen: false, // default to main-thread rendering; can be enabled explicitly when needed
...
class WorldFacade {
  ...
  constructor(options = {}){
		...
		const { onCollision, onBeforeRoll, onDieComplete, onRollComplete, onRemoveComplete, onThemeConfigLoaded, onThemeLoaded, onDiePicked, ...boxOptions } = options
		...
		this.onDiePicked = onDiePicked || this.noop
  ...
  async init() {
	...
	pickDieAt(clientX, clientY, options = {}) {
		if(!this.#DiceWorld?.pickDieAt){
			return null
		}
		const result = this.#DiceWorld.pickDieAt(clientX, clientY, options) || null
		if(result?.hit){
			this.onDiePicked(result)
		}
		return result
	}

	pickDieFromPointer(event, options = {}) {
		return this.pickDieAt(event.clientX, event.clientY, options)
	}

	setHeldState(options = {}) {
		if(!this.#DiceWorld?.setHeldState){
			return
		}
		this.#DiceWorld.setHeldState(options)
	}
