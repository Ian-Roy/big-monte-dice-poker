...
class WorldOnscreen {
	...
	pickDieAt(clientX, clientY){
		...
		const { rollId, groupId, id } = pick.pickedMesh.metadata || {}
		return {
			hit: true,
			rollId,
			groupId,
			dieId: id ?? pick.pickedMesh.id
		}
	}

	setHeldState({ids = [], held, scale = 1.08, color = '#3aa0ff'} = {}){
		if(!ids || ids.length === 0){
			return
		}
		const targetIds = new Set(ids.map(id => id?.toString()).filter(Boolean))
		Object.values(this.#dieCache).forEach(({mesh,id}) => {
			if(!mesh) return
			const meta = mesh.metadata || {}
			const rollId = meta.rollId?.toString?.()
			const cacheId = id?.toString?.()
			if(!targetIds.has(rollId) && !targetIds.has(cacheId)){
				return
			}
			this.#applyHoldState(mesh, {held, scale, color})
		})
		this.#scene.render()
	}
	...
	#applyScale(mesh, factor = null){
		const meta = mesh.metadata || {}
		const baseScale = meta?.holdState?.baseScale || meta?.baseScale || mesh.scaling || new Vector3(1,1,1)
		const {x = 1, y = 1, z = 1} = baseScale
		const holdFactor = factor !== null ? factor : (meta?.holdState?.held ? (meta?.holdState?.scale || 1) : 1)
		mesh.scaling = new Vector3(
			x * this.config.scale * holdFactor,
			y * this.config.scale * holdFactor,
			z * this.config.scale * holdFactor
		)
	}

	#applyHoldState(mesh, options){
		const {held, scale, color} = options
		if(!mesh.metadata){
			mesh.metadata = {}
		}
		if(!mesh.metadata.holdState){
			const baseScale = mesh.metadata.baseScale || mesh.scaling
			mesh.metadata.holdState = {
				baseScale: baseScale.clone ? baseScale.clone() : baseScale,
				held: false,
				scale: 1,
		...
		const holdState = mesh.metadata.holdState
		holdState.held = held
		holdState.scale = scale

		const color3 = Color3.FromHexString(color)

		// Instanced meshes can’t swap materials; prefer per-instance color if available, else overlay.
		if(mesh.isAnInstance){
			const hasInstanceColor = mesh.instancedBuffers && Object.prototype.hasOwnProperty.call(mesh.instancedBuffers, 'customColor')
			if(hasInstanceColor){
				if(!holdState.originalInstanceColor){
					const current = mesh.instancedBuffers.customColor
					holdState.originalInstanceColor = current?.clone ? current.clone() : new Color3(current?.r ?? current?.x ?? 0, current?.g ?? current?.y ?? 0, current?.b ?? current?.z ?? 0)
				}
				mesh.instancedBuffers.customColor = held
					? color3
					: holdState.originalInstanceColor || color3
			} else {
				mesh.overlayColor = color3
				mesh.overlayAlpha = held ? 0.7 : 0
				mesh.renderOverlay = !!held
			}
		} else {
			if(held){
				if(!holdState.heldMaterial && holdState.originalMaterial?.clone){
					holdState.heldMaterial = holdState.originalMaterial.clone(
						`${holdState.originalMaterial.name || 'held'}-${mesh.id}`
					)
				}
				const targetMat = holdState.heldMaterial || holdState.originalMaterial
				if(targetMat){
					targetMat.emissiveColor = color3
					targetMat.emissiveIntensity = 0.85
				}
				if(holdState.heldMaterial){
					mesh.material = holdState.heldMaterial
				}
			} else {
				if(holdState.originalMaterial){
					mesh.material = holdState.originalMaterial
				}
			}
		}
