...
class Dice {
  ...
  createInstance() {
    ...
    const baseScale = targetDie.metadata?.baseScale?.clone
      ? targetDie.metadata.baseScale.clone()
      : targetDie.metadata?.baseScale
    dieInstance.metadata = {
      ...targetDie.metadata,
      baseScale,
      rollId: this.config.rollId,
  ...
  static async loadModels(options, scene) {
    ...
    SceneLoader.ImportMeshAsync(null,null, 'data:' + JSON.stringify(modelData) , scene).then(data => {
      data.meshes.forEach(model => {
        ...
        model.metadata = {
          baseScale: model.scaling.clone ? model.scaling.clone() : model.scaling
        }
      })
      if(!has_d100 && has_d10) {
